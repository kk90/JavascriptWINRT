...
scheduler._render_calendar = function(obj, sd, conf, previous) {
	...
	d.innerHTML = toStaticHTML( "<div class='dhx_year_month'></div><div class='dhx_year_week'>" + week_template.innerHTML + "</div><div class='dhx_year_body'></div>");

	d.childNodes[0].innerHTML =  toStaticHTML(this.templates.calendar_month(sd));
	if (conf.navigation) {
		...
		for (var j = 0; j < 2; j++) {
			...
			arrow.innerHTML =  toStaticHTML(this._mini_cal_arrows[j]);
	...
	for (var k = r.length; k < 6; k++) {
		...
		for (var ri = 0; ri < r[k].childNodes.length; ri++) {
			...
			r[k].childNodes[ri].childNodes[0].innerHTML =  toStaticHTML(scheduler.date.to_fixed(++last_day_number));
		}
	}
...
scheduler.form_blocks.calendar_time = {
	...
	set_value: scheduler._wrapUnsafeFunction(function(node, value, ev) {
		...
		if (scheduler.config.full_day) {
			if (!node._full_day) {
				var html = "<label class='dhx_fullday'><input type='checkbox' name='full_day' value='true'> " + toStaticHTML(scheduler.locale.labels.full_day) + "&nbsp;</label></input>";
