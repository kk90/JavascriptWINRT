...
scheduler.createGridView=function(obj){
	...
	scheduler[name+'_view']=scheduler._wrapUnsafeFunction(function(mode){
		if (mode){
			...
			if(scheduler._min_date > new Date(0) && scheduler._max_date < (new Date(9999,1,1)))
				scheduler._els["dhx_cal_date"][0].innerHTML= toStaticHTML( scheduler.templates[name+"_date"](scheduler._min_date,scheduler._max_date));
			else
...
scheduler.grid.draw_sort_marker = scheduler._wrapUnsafeFunction(function(node, direction){
...
scheduler.grid.set_full_view = scheduler._wrapUnsafeFunction(function(mode){
	if (mode){
...
scheduler.grid._fill_grid_tab = scheduler._wrapUnsafeFunction(function(objName, sort){
	//get current date
...
scheduler.grid._print_event_row = function(ev, objName){
	...
	var html ="<tr class='dhx_grid_event"+(ev_class? ' '+ev_class:'')+"' event_id='"+toStaticHTML(ev.id)+"' " + style + ">";
	...
	for(var i =0; i < columns.length; i++){
		...
		html+= toStaticHTML("<td style='width:"+ (columns[i].width )+"px;"+cell_style.join(";")+"' "+className+">"+value+"</td>");
