...
scheduler.render_timeline_event = function (ev, attach) {
	...
	var bg_color = toStaticHTML(ev.color?("background:"+ev.color+";"):"");
	var color = toStaticHTML(ev.textColor?("color:"+ev.textColor+";"):"");
	var text = toStaticHTML(scheduler.templates.event_bar_text(ev.start_date,ev.end_date,ev));

	var html='<div event_id="'+toStaticHTML(ev.id)+'" class="'+cs+'" style="'+bg_color+''+color+'position:absolute; top:'+y+'px; height: '+hb+'px; left:'+x_start+'px; width:'+Math.max(0,x_end-x_start)+'px;'+(ev._text_style||"")+'">';
...
function y_scale(){
    ...
    return MSApp.execUnsafeLocalFunction(
        function(){return _unsafe_y_scale.apply(self, args);}
    );
}

function _unsafe_y_scale(d) {
...
function x_scale(){
    var args = arguments, self = this;
    return MSApp.execUnsafeLocalFunction(
        function(){return _unsafe_x_scale.apply(self, args);}
    );
}

function _unsafe_x_scale(h){
...
function set_full_view(){
    var args = arguments, self = this;
    return MSApp.execUnsafeLocalFunction(
        function(){return _unsafe_set_full_view.apply(self, args);}
    );
}

function _unsafe_set_full_view(mode){
	if (mode){
		scheduler.set_sizes();
...
function showToolTip(){
    var args = arguments, self = this;
    return MSApp.execUnsafeLocalFunction(
        function(){return _unsafe_showToolTip.apply(self, args);}
    );
}

function _unsafe_showToolTip(obj,pos,offset){
	...
	for (var i=0; i<evs.length; i++){
		var bg_color = toStaticHTML(evs[i].color?("background-color:"+evs[i].color+";"):"");
		var color = toStaticHTML(evs[i].textColor?("color:"+evs[i].textColor+";"):"");
		...
		html+=toStaticHTML(scheduler.templates[obj.name+"_tooltip"](evs[i].start_date, evs[i].end_date,evs[i])+"</div>");
	}
...
scheduler._render_marked_timespan = scheduler._wrapUnsafeFunction(function(options, area, unit_id) {
