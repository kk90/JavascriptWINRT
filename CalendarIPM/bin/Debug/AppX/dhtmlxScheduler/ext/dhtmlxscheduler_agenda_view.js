...
scheduler.attachEvent("onTemplatesReady",function() {
	...
	function set_full_view(mode){
		if (mode){
			...
			scheduler._els["dhx_cal_header"][0].innerHTML=toStaticHTML("<div class='dhx_agenda_line'><div>"+l.date+"</div><span style='padding-left:25px'>"+l.description+"</span></div>");
    ...
    function fill_agenda_tab(){
        return MSApp.execUnsafeLocalFunction(unsafe_fill_agenda_tab);
    }

	function unsafe_fill_agenda_tab(){
		//get current date
		...
		for (var i=0; i<events.length; i++){
			...
			html+="<div class='dhx_agenda_line"+(ev_class?' '+ev_class:'')+"' event_id='"+toStaticHTML(ev.id)+"' style='"+color+""+bg_color+""+(ev._text_style||"")+"'><div class='dhx_agenda_event_time'>"+toStaticHTML(scheduler.templates.agenda_time(ev.start_date, ev.end_date,ev))+"</div>";
			html+="<div class='dhx_event_icon icon_details'>&nbsp</div>";
			html+="<span>"+toStaticHTML(scheduler.templates.agenda_text(ev.start_date, ev.end_date, ev))+"</span></div>";
		}
