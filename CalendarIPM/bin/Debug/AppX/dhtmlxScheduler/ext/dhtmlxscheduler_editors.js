...
﻿scheduler.form_blocks['combo']={
	...
	set_value:function(node,value,ev,config){
		...
		if (!config.script_path) { // script-side filtration is used
			...
			for (var i = 0; i < config.options.length; i++) {
				...
				var single_option = [
					toStaticHTML(option.key),
					toStaticHTML(option.label),
					toStaticHTML(option.css)
				];
			...
			if (selected_id) {
				if (config.cached_options[selected_id]) {
					combo.addOption(selected_id, toStaticHTML(config.cached_options[selected_id]));
					combo.disable(1);
	...
	get_value:function(node,ev,config) {
		...
		return toStaticHTML(selected_id);
	},
	focus:function(node){
	}
};
...
scheduler.form_blocks['radio']={
	render:function(sns) {
		...
		for (var i=0; i<sns.options.length; i++) {
			...
			res += "<input id='"+id+"' type='radio' name='"+sns.name+"' value='"+toStaticHTML(sns.options[i].key)+"'><label for='"+id+"'>"+" "+toStaticHTML(sns.options[i].label)+"</label>";
			if(sns.vertical)
				res += "<br/>";
		}
		res += "</div>";
	...
	get_value:function(node,ev,config){
		...
		for(var i=0; i<radiobuttons.length; i++) {
			if(radiobuttons[i].checked) {
				return toStaticHTML(radiobuttons[i].value);
			}
		}
	},
	focus:function(node){
...
scheduler.form_blocks['checkbox']={
	render:function(sns) {
		if (scheduler.config.wide_form)
			return toStaticHTML('<div class="dhx_cal_wide_checkbox"></div>');
		else
			return '';
	},
	set_value:scheduler._wrapUnsafeFunction(function(node,value,ev,config){
		...
		if (scheduler.config.wide_form){
			node.innerHTML = toStaticHTML(label_html);
			node.nextSibling.innerHTML=toStaticHTML(check_html);
		} else 
			node.innerHTML=toStaticHTML(check_html+label_html);
	...
	get_value:function(node,ev,config){
		...
		return toStaticHTML((checkbox.checked)?(config.checked_value||true):(config.unchecked_value||false));
	},
	focus:function(node){
	}
};
