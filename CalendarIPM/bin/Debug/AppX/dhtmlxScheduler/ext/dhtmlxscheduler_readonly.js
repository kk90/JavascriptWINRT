...
﻿scheduler.attachEvent("onTemplatesReady", function() {
   ...
   /* function txt_replace(){
        ...
        return MSApp.execUnsafeLocalFunction(
            function(){return _unsafe_txt_replace.apply(self, args);}
        );
    }*/
	...
	function txt_replace(tag, d, n, text) {
		...
		for (var i = txtt.length - 1; i >= 0; i--) {
			...
			if (!text)
				...
				t.innerHTML = toStaticHTML(text(txts[i]));
    ...
    scheduler._fill_lightbox = scheduler._wrapUnsafeFunction(scheduler._fill_lightbox, scheduler);
