		...
		scheduler.form_blocks[obj.name]={
			...
			set_value:scheduler._wrapUnsafeFunction(function(node,value,ev,config){
				...
				for (var i = 0; i < options.length; i++) {
					...
					temp_option.innerHTML = toStaticHTML(options[i].label);
...
scheduler.attachEvent("onBeforeViewRender", function (render_name, y_unit, timeline){
	...
	if(render_name == "tree"){
		...
		td_content =toStaticHTML("<div class='dhx_scell_level"+y_unit.level+"'>"+div_expand+"<div class='dhx_scell_name'>"+(scheduler.templates[timeline.name+'_scale_label'](y_unit.key, y_unit.label, y_unit)||y_unit.label)+"</div></div>");
