...
scheduler.attachEvent("onTemplatesReady", function() {
	...
	var fillWeekAgendaTab = scheduler._wrapUnsafeFunction(function() {
		getColumnSizes();
		...
		scheduler._els["dhx_cal_data"][0].innerHTML =toStaticHTML(html);
		...
		for (var i = 0; i < 7; i++) {
			...
			for (var k = 0; k < evs.length; k++) {
				...
				if (ev.color)
					ev_div.style.background = toStaticHTML(ev.color);
				if (ev.textColor)
					ev_div.style.color = toStaticHTML(ev.textColor);
				...
				ev_div.innerHTML = toStaticHTML(scheduler.templates.week_agenda_event_text(ev.start_date, ev.end_date, ev, tstart, position));
	...
	scheduler.week_agenda_view = scheduler._wrapUnsafeFunction(function(mode) {
	...
	scheduler.mouse_week_agenda = scheduler._wrapUnsafeFunction(function(pos) {
