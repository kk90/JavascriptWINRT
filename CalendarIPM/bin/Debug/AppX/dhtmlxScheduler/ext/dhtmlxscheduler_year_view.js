...
(function() {
	...
	scheduler.dblclick_dhx_month_head = scheduler._wrapUnsafeFunction(function(e) {
		if (is_year_mode()) {
	...
	scheduler.showToolTip = scheduler._wrapUnsafeFunction(function(date, pos, e, src) {
		if (this._tooltip) {
		...
		for (var i = 0; i < evs.length; i++) {
			...
			html += toStaticHTML(this.templates.year_tooltip(evs[i].start_date, evs[i].end_date, evs[i]) + "</div>");
		}
	...
	scheduler._reset_year_scale = scheduler._wrapUnsafeFunction(function() {
		this._cols = [];
		this._colsS = {};
