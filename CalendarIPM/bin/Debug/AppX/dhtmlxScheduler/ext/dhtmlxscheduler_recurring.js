...
scheduler.form_blocks["recurring"] = {
		...
		function set_repeat_code(code, dates) {
			...
			switch (data[1]) {
				...
				case "":
					...
					els["date_of_end"].value = toStaticHTML(date_str(dates.end));
					break;
				default:
					els["end"][1].checked = true;
					els["occurences_count"].value = toStaticHTML(data[1]);
					break;
			}
	...
	button_click:scheduler._wrapUnsafeFunction(function(index, el, section, cont) {
		if (!cont.open && !cont.blocked) {
			cont.style.height = "115px";
			el.style.backgroundPosition = "-5px 0px";
			el.nextSibling.innerHTML = toStaticHTML(scheduler.locale.labels.button_recurring_open);
		} else {
			cont.style.height = "0px";
			el.style.backgroundPosition = "-5px 20px";
			el.nextSibling.innerHTML = toStaticHTML(scheduler.locale.labels.button_recurring);
		}
		cont.open = !cont.open;
